...
- 修复后 2018-01-01 至 2026-07-17 基线结果为净 CAGR 10.31%、最大回撤 -21.10%、Sharpe 0.774、Calmar 0.489。
...
- 核心资产袖套实现 4.17% CAGR、-12.94% 最大回撤；60%主动+40%核心组合实现 6.35% CAGR、-17.55% 最大回撤。两者作为防守和保守对照，不替换原收益基线。

当前优先级：固定规则 walk-forward/OOS > 为核心袖套补入短债/货币避险 > 产品级执行规则与映射独立验证 > 动态代表基金选择 > 纸面交易。详细结果见 `mapped_otf_strategy_report.md`。
